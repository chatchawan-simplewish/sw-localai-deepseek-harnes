...
Checkpoint 2026-09-09 03:32:13 Asia/Bangkok: the read-only `scripts/Invoke-VM105CandidateCutover.py` preflight passed (SSH0 receipt0a0b28), after independently reviewed checker corrections retained in [the preflight evidence](../../evidence/vm105-cutover-preflight-2026-09-09.json). Its mutation entry is deliberately absent and `--apply` is rejected. The proposed drop-in hash is `498ffcd058b856e054e8b144292d1831b0a3f4c4004bb0ae6d94a6e21ee0ea0a`. This does not satisfy item1's execution-helper requirement. Unfinished execution work includes identity-safe partial-file creation cleanup, outstanding systemd-job handling, bounded cross-host gates and rollback/output-failure state transitions. Do not mistake the helper filename, this preflight PASS or the design review for executable switch readiness.
...
The future helper must route remote SIGHUP/SIGTERM and EOF after the stop attempt into guarded rollback; BrokenPipe while emitting a receipt must not bypass it. The future helper must pass offline checks covering these failures, timeout, malformed/wrong-stage messages and restart races at mutation boundaries. These are unimplemented requirements, not completed tests. SIGKILL or host loss cannot guarantee automatic restoration; no new watchdog/service is introduced to imply otherwise.
