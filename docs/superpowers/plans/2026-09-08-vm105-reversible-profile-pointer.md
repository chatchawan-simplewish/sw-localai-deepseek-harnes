...
- `candidatePreparation.mutationLedger[]`: `order`, `operation`, `path`, `contentSha256`, `result`, `at`, `secretObserved`; operation is `CREATE_DIRECTORY` or `WRITE_FILE`, path is the fixed parent/root or a validated child, directory digest is `null`, final result is `PASS` or `FAILED`, and `secretObserved` is always `false`. The runner persists transient `PENDING` before the write, then replaces it with one final value.
...
- `operationLedger` entry: `at`, `actor`, `operation`, `target`, `result`, `secretObserved`; final result is `PASS` or `FAILED`, with transient `PENDING` permitted only in the pre-write receipt, and it contains no raw command output or file contents.
...
function Invoke-PreparationStep {
    ...
    try {
        foreach($operation in @($WriteOperations)){
            ...
            $receipt=[pscustomobject]@{order=@($Record.candidatePreparation.mutationLedger).Count+1;operation=$operation.operation;path=$operation.path;contentSha256=$operation.contentSha256;result='PENDING';at=(Get-Date).ToUniversalTime().ToString('o');secretObserved=$false}
            $Record.candidatePreparation.mutationLedger=@($Record.candidatePreparation.mutationLedger)+$receipt
            $opReceipt=[pscustomobject]@{at=$receipt.at;actor='dsh';operation=$operation.operation;target=$operation.path;result='PENDING';secretObserved=$false}
        ...
        if($null-ne$receipt -and $receipt.result-ceq'PENDING'){$receipt.result='FAILED';$opReceipt.result='FAILED'}
...
function Invoke-InitializerSelfTest {
    $ids=@('selector-gate','target-preflight','create-parent','create-root','write-blueprint','metadata','static-validation','secret-scan','service-unchanged','network-unchanged')
    $writeIds=@('create-parent','create-root','write-blueprint')
    foreach($failAt in $ids){
        ...
        $index=[Array]::IndexOf($ids,$failAt);$record.candidatePreparation.checks=if($index-eq0){,[object[]]@()}else{,@(0..($index-1)|ForEach-Object{[pscustomobject]@{id=$ids[$_]}})}
        $writes=[object[]]@();if($writeIds-ccontains$failAt){$path=if($failAt-ceq'create-parent'){'/home/dsh/.dsh-profiles'}elseif($failAt-ceq'create-root'){$record.target.candidateRoot}else{"$($record.target.candidateRoot)/settings.yaml"};$operation=if($failAt-ceq'write-blueprint'){'WRITE_FILE'}else{'CREATE_DIRECTORY'};$digest=if($operation-ceq'WRITE_FILE'){'A'*64}else{$null};$writes=@([pscustomobject]@{operation=$operation;path=$path;contentSha256=$digest;argv=@('synthetic-write');stdin=[byte[]]@(1)})}
        $adapter={param($argv,$stdin)$calls.Add($failAt);throw 'synthetic write failure'}
        $action={param([switch]$Blocked)if($Blocked){return [pscustomobject]@{result='BLOCKED';safeObservation=(New-SyntheticStepObservation $failAt -Blocked)}};throw 'synthetic read failure'}
        $persist={param($state)$persisted.Add([pscustomobject]@{verdict=$state.candidatePreparation.verdict;checkCount=@($state.candidatePreparation.checks).Count;mutationResults=@($state.candidatePreparation.mutationLedger.result)})}
        [void](Invoke-PreparationStep $record $failAt 'SyntheticV1' $writes $action $persist $adapter)
        Assert-Initializer ($record.candidatePreparation.verdict-ceq'BLOCKED' -and @($record.candidatePreparation.checks)[-1].id-ceq$failAt) 'failed check not persisted'
        if($writeIds-ccontains$failAt){Assert-Initializer ($record.candidatePreparation.mutationLedger[-1].result-ceq'FAILED' -and @($persisted|Where-Object{$_.mutationResults-ccontains'PENDING'}).Count-gt0 -and $persisted[-1].mutationResults[-1]-ceq'FAILED' -and $calls.Count-eq1) 'failed write receipt not persisted'}else{Assert-Initializer (@($record.candidatePreparation.mutationLedger).Count-eq0 -and $calls.Count-eq0) 'read failure created mutation receipt'}
        Assert-Initializer (@($calls|Where-Object{$_-match'cleanup|repair|service|systemd'}).Count-eq0) 'forbidden recovery call'
    }
    $record=New-SyntheticInitializerRecord;foreach($id in $ids){$writes=[object[]]@();if($writeIds-ccontains$id){$path=if($id-ceq'create-parent'){'/home/dsh/.dsh-profiles'}elseif($id-ceq'create-root'){$record.target.candidateRoot}else{"$($record.target.candidateRoot)/settings.yaml"};$writes=@([pscustomobject]@{operation=$(if($id-ceq'write-blueprint'){'WRITE_FILE'}else{'CREATE_DIRECTORY'});path=$path;contentSha256=$(if($id-ceq'write-blueprint'){'A'*64}else{$null});argv=@('synthetic-write');stdin=[byte[]]@(1)})};if(-not(Invoke-PreparationStep $record $id 'SyntheticV1' $writes {[pscustomobject]@{result='PASS';safeObservation=(New-SyntheticStepObservation $id)}} {} {})){throw 'positive sequence stopped'}}
    Assert-Initializer (@($record.candidatePreparation.checks).Count-eq10 -and @($record.candidatePreparation.mutationLedger).Count-eq3 -and @($record.candidatePreparation.mutationLedger|Where-Object result -ne PASS).Count-eq0) 'positive sequence incomplete'
...
if(-not(& $run 'selector-gate' 'SelectorGateV1' @() {param([switch]$Blocked)[pscustomobject]@{result=$(if($Blocked){'BLOCKED'}else{'PASS'});safeObservation=[pscustomobject]@{selectorDigest=$record.selectorDiscovery.selectorDigest;selectorVerdict=$(if($Blocked){'BLOCKED'}else{$record.selectorDiscovery.verdict});reviewStatus=$record.selectorDiscovery.review.status;reviewedDigest=$record.selectorDiscovery.review.reviewedDigest}}})){return}
...
if(-not(& $run 'service-unchanged' 'ServiceBaselineV1' @() {param([switch]$Blocked)if($Blocked){return [pscustomobject]@{result='BLOCKED';safeObservation=[pscustomobject]@{available=$false;activeState='';subState='';user='';group='';execStart='';workingDirectory='';umask='';result='';nRestarts=0;matchesBaseline=$false}}};$current=ConvertFrom-SanitizedUnitLines (Invoke-SshProcess @('/usr/bin/systemctl','show','deepseek-harness.service','--property=User,Group,ExecStart,WorkingDirectory,UMask,FragmentPath,DropInPaths,ActiveState,SubState,Result,NRestarts','--no-pager') $null);$before=ConvertTo-CanonicalNode ([pscustomobject]@{sourceClass=$record.selectorDiscovery.baseline.service.sourceClass;observationId=$record.selectorDiscovery.baseline.service.observationId;propertyAllowlist=$record.selectorDiscovery.baseline.service.propertyAllowlist;observations=$record.selectorDiscovery.baseline.service.observations;claim=$record.selectorDiscovery.baseline.service.claim});$after=ConvertTo-CanonicalNode ([pscustomobject]@{sourceClass=$current.sourceClass;observationId=$current.observationId;propertyAllowlist=$current.propertyAllowlist;observations=$current.observations;claim=$current.claim});$matchesBaseline=(($before|ConvertTo-Json -Depth 100 -Compress)-ceq($after|ConvertTo-Json -Depth 100 -Compress));$map=@{};foreach($item in $current.observations){$map[$item.name]=$item.value};[pscustomobject]@{result=$(if($matchesBaseline){'PASS'}else{'BLOCKED'});safeObservation=[pscustomobject]@{available=$true;activeState=$map.ActiveState;subState=$map.SubState;user=$map.User;group=$map.Group;execStart=$map.ExecStart;workingDirectory=$map.WorkingDirectory;umask=$map.UMask;result=$map.Result;nRestarts=[int]$map.NRestarts;matchesBaseline=$matchesBaseline}}})){return}
if(-not(& $run 'network-unchanged' 'NetworkBaselineV1' @() {param([switch]$Blocked)if($Blocked){return [pscustomobject]@{result='BLOCKED';safeObservation=[pscustomobject]@{available=$false;listeners=[object[]]@();localHttpStatus=0;ufwActive=$false;tcp3080Allowed=$false;directLanDenied=$false;matchesBaseline=$false}}};$listeners=ConvertFrom-ListenerLines (Invoke-SshProcess @('/usr/bin/ss','-lntH','sport = :3080') $null);$http=Get-LocalHttpObservation {param($argv)Invoke-SshProcess $argv $null};$ufw=ConvertFrom-UfwLines (Invoke-SshProcess @('/usr/bin/sudo','-n','/usr/sbin/ufw','status','numbered') $null);$direct=Get-DirectLanObservation 'controller-lan' '192.168.1.139';$before=ConvertTo-CanonicalNode ([pscustomobject]@{listeners=$record.selectorDiscovery.baseline.listeners;localHttp=$record.selectorDiscovery.baseline.localHttp;ufw=$record.selectorDiscovery.baseline.ufw;directLanDenied=$record.selectorDiscovery.baseline.directLanDenied});$after=ConvertTo-CanonicalNode ([pscustomobject]@{listeners=$listeners;localHttp=$http;ufw=$ufw;directLanDenied=$direct});$matchesBaseline=(($before|ConvertTo-Json -Depth 100 -Compress)-ceq($after|ConvertTo-Json -Depth 100 -Compress));[pscustomobject]@{result=$(if($matchesBaseline){'PASS'}else{'BLOCKED'});safeObservation=[pscustomobject]@{available=$true;listeners=$listeners;localHttpStatus=$http.statusCode;ufwActive=$ufw.active;tcp3080Allowed=$ufw.tcp3080Allowed;directLanDenied=$direct.denied;matchesBaseline=$matchesBaseline}}})){return}
...
The single production step runner above owns all ten ordered checks. It persists a `PENDING` mutation and operation receipt before every remote write, replaces both receipts with `PASS` only after that write succeeds or `FAILED` in the catch path, and persists the final `BLOCKED` check/state. Read-only failures create no mutation receipt. The catch path leaves partial candidate state untouched and stops the remaining steps; it never cleans up, repairs, retries, or invokes a service lifecycle action. Each live service/network read and parser call is inside its corresponding step action, so read failure is persisted. The comparisons omit only `selectorDiscovery.baseline.capturedAt` and the two sanitized-unit `capturedAt` values; every stable unit, listener, HTTP, UFW, and direct-LAN field is canonicalized and compared exactly.
