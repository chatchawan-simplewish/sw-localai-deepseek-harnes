...
3. The service creates one close-on-exec Unix socket pair and forks. The worker creates a session, calls `unshare(CLONE_NEWNET | CLONE_NEWPID)`, raises only `lo` through Linux ioctl, proves its network namespace differs, and forks a namespace PID-1 child. PID 1 alone creates a private mount namespace, mounts a fresh `/proc`, makes `/var/tmp` and `/run` read-only while preserving only its assigned runtime directory writable, proves its PID namespace and namespace PID, closes all descriptors except its control endpoint, credential read descriptor, and bounded receipt channel, then drops groups/GID/UID to `dsh`. The isolation claim is limited to IP/TCP; pathname `AF_UNIX` reachability remains an explicit residual.
...
Add focused tests for the fail-closed binding gate, exact `systemd-run --pipe` command without `pass_fds`, credential pipe ordering and wiping seam, topology reconstruction and sealing, worker network/PID/mount/FD allowlist/privilege-drop order, exact compact receipts, exactly two numeric-peer connections and one `sendmsg`, send/child/timeout cleanup, systemd identity and empty-cgroup proof, and secret-free receipts. The Windows host must reject real execution before any operating-system action; mock only the Linux boundary needed to test orchestration.
