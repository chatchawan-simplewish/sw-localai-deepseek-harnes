...
service profile through a reversible pointer change. The design preserves the
existing provenance-uncertain profile byte-for-byte and fails closed unless the
...
loopback port 3080. The recorded unit uses `DSH_HOME=/home/dsh/.dsh`; the
profile contains provenance-unverified state. Existing project evidence
therefore forbids opening that state or treating its ownership and restrictive
modes as proof that it is fresh or VM105-specific.
...
5. **Keep network posture unchanged.** The resulting configuration remains
   loopback-only and explicit-routing-only.
6. **Retain rollback.** The prior service pointer and profile remain available
   after success. Destruction is never part of this design.

## Architecture and boundaries

### Existing profile boundary

The existing profile, currently associated with `/home/dsh/.dsh`, is opaque.
Only already-recorded, nonsecret service-pointer metadata may be used. No new
content read, directory traversal intended to discover secret files, or copy is
permitted. Metadata inspection must be limited to what is necessary to prove
the service selector and must not enumerate or characterize secret material.
...
- that it selects the entire Harness profile, including credential and settings
  storage, rather than only a working directory or cache;
...
- explicit provider and model selection with no automatic fallback;
- no provider credentials, native OAuth state, routes, or account identities;
...
service configuration. Existing service identity, command, loopback endpoint,
working directory, restart policy, hardening, and network policy remain
unchanged unless the supported selector mechanically requires a narrower
equivalent representation. Any additional change is a material departure and
requires a revised design and approval.
...
| Static validator | Check path, ownership, modes, syntax, loopback and no-fallback policy | Secret-free candidate files and metadata | Running Harness, provider calls, secret output |
...
| Evidence record | Store commands, safe metadata, verdicts, and rollback result | Redacted nonsecret outputs | Secret values, hashes, lengths, fragments, OAuth URLs/codes |
...
inputs are installed-version evidence, existing nonsecret unit pointer
...
9. If all checks pass, retain the old profile untouched and stop before any
   credential/API access creation, OAuth/login/consent, or sensitive entry.
...
| Harness service process | `dsh:dsh` | unchanged `UMask=0077` and existing hardening |
...
| --- | --- | --- | --- | --- |
| `CURRENT_ACTIVE` | Existing service baseline verified | Read-only discovery and preparation | `CANDIDATE_STATIC_PASS` | `BLOCKED` |
...
| `CANDIDATE_STATIC_PASS` | Selector proven; candidate secret-free; static checks pass | Await separate cutover approval | `CUTOVER_APPROVED` | `BLOCKED` if state drifts |
| `CUTOVER_APPROVED` | Owner approves exact action-time cutover packet | Revalidate all inputs | `SERVICE_STOPPED` | `CURRENT_ACTIVE` |
...
| `SWITCH_ACCEPTED` | All checks pass | Record acceptance; preserve old profile | `CREDENTIAL_GATE` | `ROLLBACK_REQUIRED` if acceptance evidence is invalidated before handoff |
| `CREDENTIAL_GATE` | Candidate is active and old profile is untouched | Stop and request the existing Phase 3 credential gate | Terminal for this design | No credential action |
...
5. A content-free scan confirms no credential fields contain values, no OAuth
   state is present, and no secret-shaped material is recorded. The scan must
   report only file paths and rule verdicts, never matching values.
6. Static configuration proves loopback-only intent, explicit provider/model
   selection, and no fallback. Unsupported static validation is `NOT PROVEN`
   unless it affects safety, in which case it is `BLOCKED`.
7. The current service definition, process, listener, UFW rules, direct-LAN
   denial, and repository evidence remain unchanged after preparation.
8. No package, firewall, service, VM power, route, provider, credential, or
   OAuth mutation appears in the operation ledger.
...
8. routing remains explicit with automatic fallback disabled;
9. candidate path ownership, modes, ACLs, links, and mounts remain compliant;
10. old profile metadata is unchanged and it remains unreferenced but intact;
    and
...
| --- | --- | --- | --- | --- |
| Secret disclosure | Old profile contents are opened, copied, logged, hashed, or characterized | Credential exposure and provenance loss | Opaque-profile boundary; metadata-only evidence; secret-free scanners | `BLOCKED` if boundary cannot be honored |
...
| Route ambiguity | Candidate defaults enable fallback or wrong provider/model | Misattribution and unintended spend | Explicit-routing-only static/runtime checks; no credentials or inference | Provider behavior remains `NOT PROVEN` |
...
- The current profile is never a data source.
...
- Success leaves the old profile untouched and stops at `CREDENTIAL_GATE`.
...
- **Rollback impact:** the service returns to the prior pointer; the fresh
  candidate remains for diagnosis without touching the old profile.
